                         ...
                         const struct sockaddr_in *managers, size_t n_managers)
{
    ...
    *oc = (struct ofproto_controller) {
    ...
    /* Set the fail-mode.
     * "polka" is treated as standalone: the bridge uses OFPP_NORMAL forwarding
     * and xlate_normal() intercepts PolKA packets when polka_node_poly != 0. */
    fail_mode = !br->cfg->fail_mode
                || !strcmp(br->cfg->fail_mode, "standalone")
                || !strcmp(br->cfg->fail_mode, "polka")
                    ? OFPROTO_FAIL_STANDALONE
                    : OFPROTO_FAIL_SECURE;
    ofproto_set_fail_mode(br->ofproto, fail_mode);

    /* PolKA node polynomial — read from other_config:polka-node-id.
     * This key is populated by the Mininet PolkaSwitch helper (polka_switch.py)
     * from the nodeId keyword argument:
     *   sl = net.addSwitch('sl', cls=PolkaSwitch, nodeId='0x8005')
     * Accepts decimal or 0x-prefixed hexadecimal.
     * A value of 0 disables PolKA forwarding on this bridge.
     *
     * IMPORTANT: ofproto_set_fail_mode() does not trigger a datapath
     * revalidation, so the xlate layer (xbridge->polka_node_poly) would
     * never see the new polynomial unless we explicitly request one here.
     * Without this, xlate_polka() is never called and every PolKA packet
     * falls through to L2 flooding — only the very first packet (slow-path,
     * before any flow is installed) may reach its destination. */
    {
        const char *poly_str = smap_get_def(&br->cfg->other_config,
                                            "polka-node-id", "0");
        unsigned long poly_val = strtoul(poly_str, NULL, 0);
        struct ofproto_dpif *dpif_ofproto =
            ofproto_dpif_lookup_by_name(br->name);
        if (dpif_ofproto) {
            uint16_t new_poly = (uint16_t)poly_val;
            if (dpif_ofproto->polka_node_poly != new_poly) {
                dpif_ofproto->polka_node_poly = new_poly;
                /* Force a revalidation so xlate_ofproto_set() propagates
                 * the new polynomial to the xlate layer immediately. */
