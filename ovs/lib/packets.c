...
/* Sets the CE codepoint in 'pkt' only when the packet is ECN-capable
 * (ECT(0) or ECT(1)), per RFC 3168 section 5.3.
 *
 * Returns true if CE was applied or the packet was already CE-marked.
 * Returns false if the packet carries Not-ECT — in that case the IP header
 * is left unmodified and the caller MUST NOT forward a CE mark. */
bool
IP_ECN_set_ce_safe(struct dp_packet *pkt, bool is_ipv6)
{
    if (is_ipv6) {
        ovs_16aligned_be32 *ip6 = dp_packet_l3(pkt);
        uint32_t word = ntohl(get_16aligned_be32(ip6));

        /* ECN bits are at positions [21:20] of the first IPv6 word. */
        if (((word >> 20) & IP_ECN_MASK) == IP_ECN_NOT_ECT) {
            return false;
        }
        put_16aligned_be32(ip6, htonl(word | (IP_ECN_CE << 20)));
    } else {
        struct ip_header *nh = dp_packet_l3(pkt);
        uint8_t tos = nh->ip_tos;

        if ((tos & IP_ECN_MASK) == IP_ECN_NOT_ECT) {
            return false;
        }
        tos |= IP_ECN_CE;
        if (nh->ip_tos != tos) {
