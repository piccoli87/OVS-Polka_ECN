...
#define ETH_TYPE_POLKA         0x1234  /* PolKA source routing (polka-core.p4) */
...
/* PolKA (Polynomial Key-based Architecture) source-routing header.
 *
 * Sits immediately after the Ethernet header when etherType == ETH_TYPE_POLKA
 * (0x1234).  The 160-bit routeId encodes the entire forwarding path.
 * Each switch node computes:
 *   port = CRC16(routeId >> 16, node_poly) XOR (routeId & 0xFFFF)
 */
#define POLKA_ROUTE_ID_LEN 20   /* 160 bits */
OVS_PACKED(
struct polka_header {
    uint8_t route_id[POLKA_ROUTE_ID_LEN];
});
BUILD_ASSERT_DECL(sizeof(struct polka_header) == POLKA_ROUTE_ID_LEN);
...
/* TOS fields — ECN codepoints (RFC 3168 sect. 5).
 *
 * The two ECN bits in the IP DS field encode four codepoints:
 *   00 = Not-ECT  (not ECN-capable)
 *   01 = ECT(1)   (ECN-capable; L4S-capable per RFC 9331)
 *   10 = ECT(0)   (ECN-capable; classic)
 *   11 = CE       (Congestion Experienced — set by congested routers)
 */
...
/* Returns true if 'dsfield' indicates an ECN-capable transport (ECT(0) or
 * ECT(1)), i.e. the sender has agreed to ECN marking (RFC 3168 sect. 5). */
static inline bool
IP_ECN_is_capable(uint8_t dsfield)
{
    uint8_t ecn = dsfield & IP_ECN_MASK;
    return ecn == IP_ECN_ECT_0 || ecn == IP_ECN_ECT_1;
}

/* Returns true if 'dsfield' carries the ECT(1) codepoint, which identifies
 * L4S-capable traffic per RFC 9331. */
static inline bool
IP_ECN_is_ect1(uint8_t dsfield)
{
    return (dsfield & IP_ECN_MASK) == IP_ECN_ECT_1;
}
...
#define TCP_NS  0x100
/* AccECN (Accurate ECN) reuses the TCP NS bit as the AE (Accurate ECN echo)
 * bit during the handshake to negotiate AccECN capability.  The bit value is
 * identical to TCP_NS; both names are kept so callers can self-document intent. */
#define TCP_AE  TCP_NS
...
bool IP_ECN_set_ce_safe(struct dp_packet *pkt, bool is_ipv6);
