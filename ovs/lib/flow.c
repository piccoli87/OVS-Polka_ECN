...
miniflow_extract(struct dp_packet *packet, struct miniflow *dst)
{
    ...
    /* PolKA source routing: push the 160-bit routeId into regs[0..4] here,
     * in the metadata section, so that the miniflow values array is built in
     * strictly increasing struct-word order.  regs[0..4] occupy struct words
     * 44-46, which must come before skb_priority (word 60) and dp_hash/in_port
     * (word 61) in the values array.  MINIFLOW_ASSERT is a no-op when
     * FLOW_WC_SEQ matches, so out-of-order pushes silently corrupt the
     * miniflow — this early peek avoids that.
     *
     * We peek at the raw Ethernet frame without consuming 'data' or 'size'.
     * Note: VLAN-tagged PolKA frames are not handled here (the etherType peek
     * would see 0x8100 instead of 0x1234). */
    if (packet_type == htonl(PT_ETH)
        && size >= sizeof(struct eth_header) + POLKA_ROUTE_ID_LEN) {
        const uint8_t *pkt = (const uint8_t *)data;
        ovs_be16 etype;
        memcpy(&etype, pkt + 2 * ETH_ADDR_LEN, sizeof etype);
        if (etype == htons(ETH_TYPE_POLKA)) {
            const uint8_t *rid = pkt + sizeof(struct eth_header);
#define POLKA_RID_WORD(b, n) \
    (((uint32_t)(b)[(n)*4+0] << 24) | \
     ((uint32_t)(b)[(n)*4+1] << 16) | \
     ((uint32_t)(b)[(n)*4+2] <<  8) | \
      (uint32_t)(b)[(n)*4+3])
            miniflow_push_uint32(mf, regs[0], POLKA_RID_WORD(rid, 0));
            miniflow_push_uint32(mf, regs[1], POLKA_RID_WORD(rid, 1));
            miniflow_push_uint32(mf, regs[2], POLKA_RID_WORD(rid, 2));
            miniflow_push_uint32(mf, regs[3], POLKA_RID_WORD(rid, 3));
            miniflow_push_uint32(mf, regs[4], POLKA_RID_WORD(rid, 4));
            miniflow_push_uint32(mf, regs[5], 0); /* complete word 46 */
#undef POLKA_RID_WORD
        }
    }
    ...
    if (OVS_LIKELY(dl_type == htons(ETH_TYPE_IP))) {
        ...
        } else if (dl_type == htons(ETH_TYPE_POLKA)) {
            /* Push a non-zero sentinel into nw_src so the kernel megaflow
             * key never matches subsequent PolKA packets: the kernel always
             * parses nw_src=0 for ethertype 0x1234 (unknown L3), so any
             * non-zero nw_src in the megaflow causes a miss → upcall →
             * correct per-routeId routing via xlate_polka every time. */
            miniflow_push_be32(mf, nw_src, OVS_BE32_MAX);
            miniflow_push_be32(mf, nw_dst, 0);
        }
        goto out;
    }
