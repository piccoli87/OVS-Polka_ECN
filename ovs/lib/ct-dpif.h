...
/* ECN negotiation state reported in ct_dpif_protoinfo.tcp.ecn_flags.
 *
 * CT_DPIF_ECN_INIT      — the TCP initiator requested ECN by sending a SYN
 *                         with both ECE and CWR set (RFC 3168 sect. 6.1.1).
 * CT_DPIF_ECN_CONFIRMED — the responder acknowledged ECN capability by
 *                         sending a SYN-ACK with ECE set and CWR clear
 *                         (RFC 3168 sect. 6.1.1).
 * CT_DPIF_ECN_ACCECN    — AccECN was requested: the SYN additionally carried
 *                         the AE (= NS) bit, signalling Accurate ECN support. */
#define CT_DPIF_ECN_NONE      0x00
#define CT_DPIF_ECN_INIT      0x01
#define CT_DPIF_ECN_CONFIRMED 0x02
#define CT_DPIF_ECN_ACCECN    0x04
...
struct ct_dpif_protoinfo {
    ...
    union {
        struct {
            ...
            uint8_t flags_orig;
            uint8_t flags_reply;
            uint8_t ecn_flags;  /* CT_DPIF_ECN_* — ECN/AccECN negotiation state */
            uint8_t pad;
        } tcp;
        struct {
            uint8_t state;
