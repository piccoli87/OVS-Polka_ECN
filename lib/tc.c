...
/* Reads the root qdisc stats for the interface identified by 'ifindex'.
 * On success, writes the current queue depth in packets to '*qlen' and the
 * cumulative overflow-drop count to '*drops', then returns 0.
 * Returns a positive errno on failure (ENODEV, ENOENT, EOPNOTSUPP). */
int
tc_get_qdisc_stats(int ifindex, uint32_t *qlen, uint32_t *drops)
{
    struct ofpbuf request, *reply;
    struct tcmsg *tcmsg;
    int error;

    tcmsg = tc_make_request(ifindex, RTM_GETQDISC, NLM_F_ECHO, &request);
    if (!tcmsg) {
        return ENODEV;
    }
    tcmsg->tcm_handle = 0;
    ...
    if (error || !reply || !reply->size) {
        ofpbuf_delete(reply);
        return error ? error : ENOENT;
    }

    static const struct nl_policy stats2_policy[] = {
        [TCA_STATS2] = { .type = NL_A_NESTED, .optional = true },
    };
    struct nlattr *ta[ARRAY_SIZE(stats2_policy)];

    struct ofpbuf b = ofpbuf_const_initializer(reply->data, reply->size);
    ofpbuf_pull(&b, NLMSG_HDRLEN + sizeof(struct tcmsg));

    if (!nl_policy_parse(&b, 0, stats2_policy, ta, ARRAY_SIZE(stats2_policy))
        || !ta[TCA_STATS2]) {
        ofpbuf_delete(reply);
        return EOPNOTSUPP;
    }

    static const struct nl_policy sq_policy[] = {
        [TCA_STATS_QUEUE] = { .type = NL_A_UNSPEC, .optional = false,
                              .min_len = sizeof(struct gnet_stats_queue) },
    };
    struct nlattr *sa[ARRAY_SIZE(sq_policy)];

    if (!nl_parse_nested(ta[TCA_STATS2], sq_policy, sa,
                         ARRAY_SIZE(sq_policy))) {
        ofpbuf_delete(reply);
        return EOPNOTSUPP;
    }

    const struct gnet_stats_queue *gsq = nl_attr_get(sa[TCA_STATS_QUEUE]);
    *qlen  = gsq->qlen;
    *drops = gsq->drops;
