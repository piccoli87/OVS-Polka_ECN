                                     ...
                                     const struct ct_dpif_protoinfo *protoinfo)
{
    ...
    if (protoinfo->tcp.ecn_flags) {
        ds_put_cstr(ds, ",ecn=");
        if (protoinfo->tcp.ecn_flags & CT_DPIF_ECN_CONFIRMED) {
            ds_put_cstr(ds, protoinfo->tcp.ecn_flags & CT_DPIF_ECN_ACCECN
                        ? "accecn" : "confirmed");
        } else if (protoinfo->tcp.ecn_flags & CT_DPIF_ECN_INIT) {
            ds_put_cstr(ds, "init");
        }
    }
}
