...
struct dp_netdev {
    ...
    /* AQM CoDel parameters (µs). 0 means "use RFC 8289 default"
     * (5 ms target / 100 ms interval). Configured via OVSDB:
     *   ovs-vsctl set Open_vSwitch . \
     *     other_config:aqm-codel-target-us=<value>
     *     other_config:aqm-codel-interval-us=<value> */
    atomic_llong aqm_target_us;
    atomic_llong aqm_interval_us;

    /* INT (In-band Network Telemetry) transit node parameters.
     * Configured via OVSDB other_config keys on the Open_vSwitch table:
     *
     *   int-node-id  — 32-bit node identifier, unique within the INT domain.
     *                  0 disables INT transit processing (default).
     *   int-dscp     — DSCP value used to detect INT packets in IPv4 TOS.
     *                  Default: INT_DSCP_DEFAULT (0x17, as in INT spec v2.1).
     *
     * Example:
     *   ovs-vsctl set Open_vSwitch . \
     ...
     */
    atomic_uint32_t int_node_id;   /* 0 = INT transit disabled */
    atomic_uint32_t int_dscp;      /* DSCP marker for INT packets */
...
struct tx_port {
    ...
    /* CoDel AQM state — owned exclusively by the PMD thread for this port. */
    struct aqm_codel_state aqm;
};
    ...
    OVS_REQUIRES(dp_netdev_mutex)
{
    ...
    *CONST_CAST(const char **, &dp->full_name) = xasprintf("%s@%s",
    ...
    atomic_init(&dp->aqm_target_us, 0LL);    /* 0 = RFC 8289 default (5 ms) */
    atomic_init(&dp->aqm_interval_us, 0LL);  /* 0 = RFC 8289 default (100 ms) */
    atomic_init(&dp->int_node_id, 0U);       /* 0 = INT transit disabled */
    atomic_init(&dp->int_dscp, (unsigned)INT_DSCP_DEFAULT);
...
dpif_netdev_set_config(struct dpif *dpif, const struct smap *other_config)
{
    ...
    /* AQM CoDel tuning — applied at the next PMD TX-port rebuild.
     * Keys (µs, 0 = RFC 8289 default):
     *   aqm-codel-target-us   — sojourn target   (default 5 000 µs)
     *   aqm-codel-interval-us — marking interval (default 100 000 µs) */
    long long aqm_target   = (long long)smap_get_ullong(other_config,
                                                        "aqm-codel-target-us",
                                                        0ULL);
    long long aqm_interval = (long long)smap_get_ullong(other_config,
                                                        "aqm-codel-interval-us",
                                                        0ULL);
    long long cur_aqm_target, cur_aqm_interval;
    atomic_read_relaxed(&dp->aqm_target_us,   &cur_aqm_target);
    atomic_read_relaxed(&dp->aqm_interval_us, &cur_aqm_interval);
    if (aqm_target != cur_aqm_target || aqm_interval != cur_aqm_interval) {
        atomic_store_relaxed(&dp->aqm_target_us,   aqm_target);
        atomic_store_relaxed(&dp->aqm_interval_us, aqm_interval);
        VLOG_INFO("AQM CoDel: target=%lld µs interval=%lld µs "
                  "(0 = RFC 8289 default)", aqm_target, aqm_interval);
        dp_netdev_request_reconfigure(dp);
    }

    /* INT transit node tuning.
     * Keys:
     *   int-node-id  — 32-bit node identifier (0 = disabled, default)
     *   int-dscp     — DSCP value that marks INT packets (default 0x17) */
    unsigned int int_node_id = (unsigned int)smap_get_ullong(other_config,
                                                             "int-node-id",
                                                             0ULL);
    unsigned int int_dscp    = (unsigned int)smap_get_ullong(other_config,
                                                             "int-dscp",
                                                             INT_DSCP_DEFAULT);
    /* Clamp DSCP to valid 6-bit range. */
    int_dscp &= 0x3fu;

    unsigned int cur_int_node_id, cur_int_dscp;
    atomic_read_relaxed(&dp->int_node_id, &cur_int_node_id);
    atomic_read_relaxed(&dp->int_dscp,    &cur_int_dscp);
    if (int_node_id != cur_int_node_id || int_dscp != cur_int_dscp) {
        atomic_store_relaxed(&dp->int_node_id, int_node_id);
        atomic_store_relaxed(&dp->int_dscp,    int_dscp);
        if (int_node_id) {
            VLOG_INFO("INT transit: node-id=0x%08x dscp=0x%02x",
                      int_node_id, int_dscp);
        } else {
            VLOG_INFO("INT transit: disabled (int-node-id=0)");
        }
    }
                                   ...
                                   struct tx_port *p)
{
    ...
    /* --- CoDel AQM (RFC 8289) ---
     *
     * Measure batch sojourn time and, when congestion is detected:
     *   - Mark CE on the first ECN-capable packet (IP_ECN_set_ce_safe).
     *   - If no ECN-capable packet exists, drop packet[0] (Not-ECT).
     *
     * We act only when the batch holds ≥ 2 packets to avoid emptying it
     * entirely here (an empty batch would cause a division-by-zero in the
     * cycle-accounting loop below). */
    if (output_cnt >= 2) {
        int drop_idx = aqm_codel_run(&p->aqm, &p->output_pkts,
                                     pmd->ctx.now);
        if (drop_idx == 0) {
            /* Drop packet[0] and compact both the batch and the rxqs array. */
            int n = (int)p->output_pkts.count;
            dp_packet_delete(p->output_pkts.packets[0]);
            for (int k = 0; k + 1 < n; k++) {
                p->output_pkts.packets[k] = p->output_pkts.packets[k + 1];
                p->output_pkts_rxqs[k]    = p->output_pkts_rxqs[k + 1];
            }
            p->output_pkts.count--;
            output_cnt--;
        }
    ...
    /* --- INT-MD Transit Node (INT Dataplane Spec v2.1 §5.2.2 / §5.8) ---
     *
     * When int_node_id != 0, inspect every packet in the TX batch.
     * INT packets are identified by their IPv4 DSCP field (int_dscp).
     * For each INT-MD packet, insert this node's metadata block immediately
     * after the INT-MD header, decrement Remaining Hop Count, and update
     * IP/L4 checksums.
     *
     * PolKA packets (etherType 0x1234) are skipped: they carry no IP header,
     * so the DSCP check would read into the PolKA routeId bytes.
     *
     * Metadata sources at TX-flush time:
     *   node_id       — from other_config:int-node-id
     *   ingress_ifid  — pkt->md.in_port.odp_port (set at RX)
     *   egress_ifid   — p->port->port_no (this TX port)
     *   batch_age_us  — now − batch_enqueue_us (CoDel batch birth timestamp)
     *   egress_ts_us  — pmd->ctx.now
     *   queue_occupancy — tc root qdisc backlog in packets (via RTM_GETQDISC)
     *   buf_occupancy   — tc root qdisc cumulative overflow drops (24-bit),
     *                     packed with egress_ifid (8-bit) as buffer ID
     */
    unsigned int int_node_id_val;
    atomic_read_relaxed(&pmd->dp->int_node_id, &int_node_id_val);

    if (int_node_id_val != 0) {
        unsigned int int_dscp_val;
        atomic_read_relaxed(&pmd->dp->int_dscp, &int_dscp_val);

        uint16_t egress_ifid  = (uint16_t)odp_to_u32(p->port->port_no);
        int64_t  now_us       = time_usec();
        int64_t  batch_age_us = now_us - p->aqm.batch_enqueue_us;
        int64_t  egress_ts_us = now_us;

        /* Read real tc qdisc stats for the egress interface. */
        uint32_t queue_occ = 0;
        uint32_t buf_occ   = 0;
        ...
        DP_PACKET_BATCH_FOR_EACH (ii, int_pkt, &p->output_pkts) {
            /* Skip PolKA packets — no IP header to check DSCP against. */
            if (dp_packet_is_eth(int_pkt)) {
                const struct eth_header *eth_h = dp_packet_data(int_pkt);
                if (eth_h->eth_type == htons(ETH_TYPE_POLKA)) {
                    continue;
                }
            }

            if (!int_is_int_packet(int_pkt, (uint8_t)int_dscp_val)) {
                continue;
            }

            uint16_t ingress_ifid =
                (uint16_t)odp_to_u32(int_pkt->md.in_port.odp_port);

            int_transit_process(int_pkt,
                                int_node_id_val,
                                ingress_ifid,
                                egress_ifid,
                                batch_age_us,
    ...
    OVS_REQUIRES(pmd->port_mutex)
{
    ...
    long long aqm_t, aqm_i;
    atomic_read_relaxed(&pmd->dp->aqm_target_us,   &aqm_t);
    atomic_read_relaxed(&pmd->dp->aqm_interval_us, &aqm_i);
    aqm_codel_init(&tx->aqm, aqm_t, aqm_i);
                         ...
                         bool should_steal, odp_port_t port_no)
{
    ...
    if (dp_packet_batch_is_empty(&p->output_pkts)) {
        pmd->n_output_batches++;
        /* Record the batch birth time for CoDel sojourn measurement. */
        p->aqm.batch_enqueue_us = pmd->ctx.now;
    }
