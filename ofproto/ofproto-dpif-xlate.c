...
struct xbridge {
    ...
    /* PolKA source-routing node polynomial.  Zero means PolKA is disabled.
     * Set from ofproto_dpif.polka_node_poly via xlate_ofproto_set(). */
    uint16_t polka_node_poly;
};
...
static void clone_xlate_actions(const struct ofpact *, size_t ofpacts_len,
                                struct xlate_ctx *, bool, bool);
static void xlate_polka(struct xlate_ctx *);
                  ...
                  uint16_t polka_node_poly)
{
    ...
    xbridge->polka_node_poly = polka_node_poly;
}
...
xlate_xbridge_copy(struct xbridge *xbridge)
{
    ...
    xlate_xbridge_set(new_xbridge,
                      ...
                      xbridge->addr, xbridge->polka_node_poly);
...
/* xlate_polka - forward a PolKA source-routed packet.
 *
 * Called from xlate_normal() when dl_type == ETH_TYPE_POLKA and the bridge
 * has a non-zero polka_node_poly.
 *
 * CRITICAL: In the kernel datapath, the ODP key that arrives with each
 * upcall never includes flow->regs (the kernel does not parse ethertype
 * 0x1234).  Relying on flow->regs would always give zeros, causing
 * xlate_polka to compute the same (wrong) output port for every packet.
 * The routeId is read directly from the raw packet bytes instead.
 */
static void
xlate_polka(struct xlate_ctx *ctx)
{
    struct flow_wildcards *wc = ctx->wc;
    const struct xbridge *xbr = ctx->xbridge;

    /* Extract the 20-byte PolKA routeId from the raw packet.
     * The upcall always carries the original packet (ctx->xin->packet),
     * so we walk past the Ethernet header (and optional VLAN tags) to
     * reach the routeId bytes. */
    uint8_t route_id[POLKA_ROUTE_ID_LEN];
    bool got_rid = false;

    const struct dp_packet *pkt = ctx->xin->packet;
    if (pkt) {
        const uint8_t *data = (const uint8_t *)dp_packet_data(pkt);
        size_t size = dp_packet_size(pkt);
        /* Point past the two MAC addresses to the ethertype field. */
        const uint8_t *p = data + 2 * ETH_ADDR_LEN;
        size_t rem    = size  - 2 * ETH_ADDR_LEN;

        /* Skip optional VLAN / 802.1ad tags. */
        while (rem >= 2) {
            ovs_be16 etype;
            memcpy(&etype, p, sizeof etype);
            p   += 2;
            rem -= 2;
            if (etype == htons(ETH_TYPE_POLKA)) {
                if (rem >= POLKA_ROUTE_ID_LEN) {
                    memcpy(route_id, p, POLKA_ROUTE_ID_LEN);
                    got_rid = true;
                }
                break;
            } else if (etype == htons(ETH_TYPE_VLAN) ||
                       etype == htons(0x88a8u)) {
                /* Skip the 2-byte VLAN TCI field. */
                if (rem < 2) {
                    break;
                }
                p   += 2;
                ...
                break; /* unexpected ethertype before PolKA header */
            }
        }
    }

    if (!got_rid) {
        /* Fallback: reconstruct routeId from flow->regs, which miniflow_extract()
         * populates in the userspace-DP (datapath=netdev) path. */
        struct flow *flow = &ctx->xin->flow;
        for (int i = 0; i < 5; i++) {
            uint32_t word = flow->regs[i];
            route_id[i * 4 + 0] = (word >> 24) & 0xff;
            route_id[i * 4 + 1] = (word >> 16) & 0xff;
            route_id[i * 4 + 2] = (word >>  8) & 0xff;
            route_id[i * 4 + 3] =  word        & 0xff;
        }
    }

    uint16_t port_no = polka_compute_nhop(route_id, xbr->polka_node_poly);
    ofp_port_t ofp_port = u16_to_ofp(port_no);

    xlate_report(ctx, OFT_DETAIL,
                 "PolKA: poly=0x%04x rid[0..1]=0x%02x%02x port=%u src=%s",
                 xbr->polka_node_poly, route_id[0], route_id[1],
                 (unsigned)port_no, got_rid ? "packet" : "regs");

    /* PolKA is stateless: should_install_flow() never caches a kernel
     * megaflow for 0x1234 packets.  These wc masks are kept only for the
     * userspace-DP (datapath=netdev) path where DPCLS uses them. */
    ...
    if (!xport) {
        xlate_report(ctx, OFT_WARN,
                     "PolKA: port %u not found on bridge, dropping",
                     (unsigned)port_no);
        return;
    }
...
xlate_normal(struct xlate_ctx *ctx)
{
    ...
    /* PolKA source routing: intercept before any L2/VLAN processing.
     * Set wc->masks.dl_type before reading flow->dl_type so the installed
     * megaflow correctly matches on ethertype (OVS convention: mask every
     * field that influences a forwarding decision).
     *
     * When polka_node_poly == 0 (race window before config propagates to
     * xbridge), return DROP with a generic {in_port,dl_type}→drop megaflow
     * rather than falling through to L2 code.  L2 would install a MAC-
     * specific megaflow that survives revalidation; the revalidator then
     * re-runs xlate with regs=0 (wildcarded in the old key), computes the
     * wrong next-hop port, and installs a permanent DROP.  A generic drop is
     * evicted and replaced by xlate_polka as soon as the config propagates. */
    wc->masks.dl_type = OVS_BE16_MAX;
    if (flow->dl_type == htons(ETH_TYPE_POLKA)) {
        if (ctx->xbridge->polka_node_poly != 0) {
            xlate_polka(ctx);
        }
        return;
    }
