...
struct ofproto_dpif {
     ...
     * Non-zero value enables PolKA mode: when a packet arrives with
     * etherType == ETH_TYPE_POLKA (0x1234), the output port is determined by
     * CRC16(routeId >> 16, polka_node_poly) XOR (routeId & 0xFFFF).
     * Configured via other_config:polka-node-id in OVSDB. */
